"""Gateway abstraction layer for OpenClaw phase-out (S-31).

This module provides an abstract interface for LLM gateway communication.
Currently implemented by OpenClawGateway; NativeGateway planned for v1.3.
"""
...
class OpenClawGateway(GatewayInterface):
    """Current implementation — delegates to OpenClaw/clawdbot container.

    The clawdbot service runs at port 18789 (docker-compose.yml:49).
    Config generated by aria_models/openclaw_config.py.
    """

    def __init__(self, base_url: str = "http://clawdbot:18789"):
    ...
    async def send_message(
        ...
        try:
            # TODO: wire to actual OpenClaw REST or WebSocket API
            raise NotImplementedError(
                "OpenClawGateway.send_message() — wire to OpenClaw API in v1.3"
            )
        finally:
    ...
    async def get_system_prompt(self) -> str:
        ...
        prompt_file = os.environ.get("OPENCLAW_SYSTEM_PROMPT_FILE", "")
        if prompt_file:
            p = Path(prompt_file)
            if p.exists():
                return p.read_text(encoding="utf-8")
        return "(default OpenClaw prompt — not customized)"

    async def list_tools(self) -> List[Dict[str, Any]]:
        # TODO: query OpenClaw /tools endpoint
        raise NotImplementedError("OpenClawGateway.list_tools() — v1.3")
