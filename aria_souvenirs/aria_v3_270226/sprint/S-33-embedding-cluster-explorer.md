...
curl -s "http://localhost:${ARIA_API_PORT:-8000}/api/memories/embedding-projection?limit=50&method=pca" | python3 -m json.tool | head -20
...
curl -s "http://localhost:${ARIA_API_PORT:-8000}/api/memories/embedding-projection?limit=100" | python3 -c "
import json, sys
