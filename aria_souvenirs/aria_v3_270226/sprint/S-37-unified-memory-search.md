...
curl -s "http://localhost:${ARIA_API_PORT:-8000}/api/memory-search?query=security&limit=5" | python3 -m json.tool | head -25
# EXPECTED: JSON with "results" array containing mixed types, "by_type" breakdown

# 2. Multiple types in results:
curl -s "http://localhost:${ARIA_API_PORT:-8000}/api/memory-search?query=error&limit=20" | python3 -c "
import json, sys
...
curl -s "http://localhost:${ARIA_API_PORT:-8000}/api/memory-search?query=pattern&limit=10" | python3 -c "
import json, sys
d = json.load(sys.stdin)
