# S3-05: Per-Agent Heartbeats via Scheduler
**Epic:** E2 — Scheduler & Heartbeat | **Priority:** P0 | **Points:** 3 | **Phase:** 2

## Problem
The existing `aria_mind/heartbeat.py` runs as a single-agent loop on a fixed interval. In the engine world, each agent needs its own heartbeat — the main agent beats every 30 seconds, sub-agents every 5 minutes. Heartbeats should be managed by the scheduler (S3-01), write to `engine_agent_state.last_active_at`, and trigger health alerts when beats are missed.

Reference: `aria_mind/heartbeat.py` (lines 1-296) defines the `Heartbeat` class with `_beat()`, self-healing, and autonomous actions. `aria_engine.agent_state` has `last_active_at` and `status` columns. The `EngineScheduler` (S3-01) dispatches to agents via `AgentPool`.

## Root Cause
The current heartbeat is monolithic — one loop for the entire `AriaMind` instance. It checks `soul`, `memory`, and `cognition` subsystems but has no concept of multiple agents. With the engine's `AgentPool` managing 6+ agents, each agent needs independent health monitoring. The scheduler is the natural place to schedule per-agent heartbeats since it already manages recurring jobs.

## Fix
### `aria_engine/heartbeat.py`
```python
"""
...
|---|-----------|---------|-------|
| 1 | 5-layer (DB→ORM→API→api_client→Skills→Agents) | ✅ | Heartbeat writes to DB, checks agents |
| 2 | .env for secrets (zero in code) | ✅ | DATABASE_URL from EngineConfig |
| 3 | models.yaml single source of truth | ❌ | No model references |
| 4 | Docker-first testing | ✅ | Requires PostgreSQL for agent_state writes |
| 5 | aria_memories only writable path | ❌ | No file writes — state in DB |
| 6 | No soul modification | ❌ | No soul access |

## Dependencies
- S3-01 (EngineScheduler — registers heartbeat jobs)
- S1-05 (Alembic migration — agent_state table)
- S4-01 (AgentPool — for agent health checks; can stub)

## Verification
```bash
# 1. Module imports:
python -c "from aria_engine.heartbeat import AgentHeartbeatManager, AgentHeartbeatConfig; print('OK')"
# EXPECTED: OK

# 2. Config creation:
python -c "
from aria_engine.heartbeat import AgentHeartbeatConfig
main = AgentHeartbeatConfig('main', interval_seconds=30)
print(f'Schedule: {main.schedule_expression}, Miss threshold: {main.miss_threshold_seconds}s')
sub = AgentHeartbeatConfig('aria-talk', interval_seconds=300)
print(f'Schedule: {sub.schedule_expression}, Miss threshold: {sub.miss_threshold_seconds}s')
"
# EXPECTED: Schedule: 30s, Miss threshold: 90s
#           Schedule: 5m, Miss threshold: 900s

# 3. Manager instantiation:
python -c "
from aria_engine.config import EngineConfig
from aria_engine.heartbeat import AgentHeartbeatManager
from sqlalchemy.ext.asyncio import create_async_engine
engine = create_async_engine('postgresql+asyncpg://localhost/aria')
mgr = AgentHeartbeatManager(EngineConfig(), engine)
mgr.configure_agent('main')
mgr.configure_agent('aria-talk')
print(mgr.get_status())
"
# EXPECTED: {'configured_agents': 2, 'beat_counts': {...}, ...}
```

## Prompt for Agent
```
Implement per-agent heartbeats managed by the EngineScheduler.

FILES TO READ FIRST:
- aria_mind/heartbeat.py (full file — existing heartbeat logic to port)
- aria_engine/scheduler.py (created in S3-01 — add_job method for registering heartbeats)
- MASTER_PLAN.md (lines 140-160 — agent_state table with last_active_at)
- aria_engine/config.py (created in S1-01 — EngineConfig)

STEPS:
1. Read all files above
2. Create aria_engine/heartbeat.py with AgentHeartbeatManager class
3. Implement AgentHeartbeatConfig — per-agent interval and miss threshold
4. Implement configure_agent() — set intervals (30s main, 5min others)
5. Implement register_all_agents() — discover agents from DB, register with scheduler
6. Implement heartbeat_handler() — single beat cycle with health checks
7. Implement _update_last_active() — write to agent_state
8. Implement check_all_health() — detect missed beats, mark unhealthy
9. Run verification commands

CONSTRAINTS:
- Constraint 1: Heartbeat manager is infrastructure — writes to DB, checks agents
- Main agent: 30s interval, sub-agents: 5min interval (configurable)
- Mark agent unhealthy if heartbeat missed > 3x interval
- No actual LLM calls in heartbeat — only connectivity checks
```
