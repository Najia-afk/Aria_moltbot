# S4-01: Async Agent Lifecycle Management
**Epic:** E3 — Agent Pool | **Priority:** P0 | **Points:** 5 | **Phase:** 3

## Problem
Agents are currently instantiated ad-hoc by `AgentCoordinator` during `initialize_agents()` — they're simple `LLMAgent` objects in a dict with no lifecycle management. The engine needs a proper `AgentPool` that loads agents from the database, manages their lifecycle (spawn/track/terminate), enforces concurrency limits, and provides a unified interface for the scheduler, chat engine, and API to interact with agents.

Reference: `aria_agents/coordinator.py` (lines 260-290) shows `initialize_agents()` creating `LLMAgent` instances from `AgentConfig` loaded from `AGENTS.md`. The `aria_engine.agent_state` table stores agent runtime state. `MAX_CONCURRENT_AGENTS = 5` is the concurrency limit.

## Root Cause
The current `AgentCoordinator` was designed for single-process in-memory operation within `AriaMind`. It has no database backing, no lifecycle events, and no concept of agent "status" beyond what's tracked in memory. The engine needs database-persisted agent state, async lifecycle management with TaskGroup, and clean integration points for the scheduler and chat engine.
...
class AgentPool:
    ...
    Usage:
        ...
        # Get an agent
        agent = pool.get_agent("main")
        response = await agent.process("What are today's goals?")

        # Spawn a new agent
        await pool.spawn_agent("aria-researcher", model="qwen3-mlx")

        # List all agents
        for summary in pool.list_agents():
            print(summary)

        # Terminate an agent
        await pool.terminate_agent("aria-researcher")
    """
...
|---|-----------|---------|-------|
| 1 | 5-layer (DB→ORM→API→api_client→Skills→Agents) | ✅ | AgentPool at infrastructure layer, manages agents |
| 2 | .env for secrets (zero in code) | ✅ | DATABASE_URL from EngineConfig |
| 3 | models.yaml single source of truth | ✅ | Agent models resolved via LLM gateway (which uses models.yaml) |
...
- S1-02 (LLM Gateway — for agent.process() calls)
- S1-05 (Alembic migration — agent_state table)

## Verification
```bash
# 1. Module imports:
python -c "from aria_engine.agent_pool import AgentPool, EngineAgent; print('OK')"
# EXPECTED: OK

# 2. EngineAgent creation:
python -c "
from aria_engine.agent_pool import EngineAgent
agent = EngineAgent(agent_id='main', model='qwen3-mlx', display_name='Aria Main')
print(agent.get_summary())
"
# EXPECTED: {'agent_id': 'main', 'model': 'qwen3-mlx', 'status': 'idle', ...}

# 3. AgentPool instantiation:
python -c "
from aria_engine.config import EngineConfig
from aria_engine.agent_pool import AgentPool
from sqlalchemy.ext.asyncio import create_async_engine
engine = create_async_engine('postgresql+asyncpg://localhost/aria')
pool = AgentPool(EngineConfig(), engine)
print(pool.get_status())
"
# EXPECTED: {'total_agents': 0, 'max_concurrent': 5, ...}
```

## Prompt for Agent
```
Implement the AgentPool for Aria Engine — async agent lifecycle management.

FILES TO READ FIRST:
- aria_agents/coordinator.py (full file — existing AgentCoordinator, LLMAgent)
- aria_agents/base.py (AgentConfig, AgentMessage, BaseAgent classes)
- aria_engine/llm_gateway.py (created in S1-02 — LLMGateway.complete())
- aria_engine/config.py (created in S1-01 — EngineConfig)
- MASTER_PLAN.md (lines 140-160 — agent_state table schema)

STEPS:
1. Read all files above
2. Create aria_engine/agent_pool.py
3. Implement EngineAgent dataclass with process(), clear_context(), get_summary()
4. Implement AgentPool with load_agents() — reads agent_state table
5. Implement spawn_agent() — INSERT to DB + create EngineAgent
6. Implement terminate_agent() — cancel tasks, persist state, remove
7. Implement process_with_agent() — with concurrency semaphore
8. Implement run_parallel() — asyncio.TaskGroup for concurrent tasks
9. Implement _persist_agent_state() — write state changes to DB
10. Run verification commands

CONSTRAINTS:
- Constraint 1: AgentPool manages agent lifecycle, delegates LLM calls to gateway
- Max 5 concurrent agents (MAX_CONCURRENT_AGENTS = 5)
- asyncio.TaskGroup for structured concurrency (Python 3.11+)
- Agent state persisted to DB after each interaction
```
