# S4-02: Per-Agent Session Isolation
**Epic:** E3 — Agent Pool | **Priority:** P0 | **Points:** 3 | **Phase:** 3

## Problem
Currently all agents share the same session space — there's no filtering by `agent_id` when listing or creating sessions. In the engine, each agent must have its own isolated set of sessions. If `aria-talk` creates a session for social posting, that session must not appear in `main`'s session list or leak context into unrelated conversations.

Reference: `aria_engine.chat_sessions` has an `agent_id` column (from S1-05 migration). `aria_engine.chat_messages` references `session_id`. The `AgentPool` (S4-01) manages `EngineAgent` instances with `current_session_id`.

## Root Cause
OpenClaw managed agents in separate directories (`/root/.openclaw/agents/<name>/sessions/`), providing filesystem-level isolation. The new PostgreSQL-based session system has all sessions in one table. Without proper filtering and isolation at the query level, agents could see each other's sessions and messages.

## Fix
### `aria_engine/session_isolation.py`
```python
"""
...
class AgentSessionScope:
    ...
    async def create_session(
        ...
        logger.info(
            ...
            session_id, self.agent_id, session_type,
        )
...
|---|-----------|---------|-------|
| 1 | 5-layer (DB→ORM→API→api_client→Skills→Agents) | ✅ | Session isolation at infrastructure layer |
...
| 5 | aria_memories only writable path | ❌ | No file writes — all DB |
| 6 | No soul modification | ❌ | No soul access |

## Dependencies
- S1-05 (Alembic migration — chat_sessions, chat_messages tables with agent_id)
- S4-01 (AgentPool — uses AgentSessionScope per agent)
...
# 2. Factory pattern:
python -c "
from aria_engine.session_isolation import SessionIsolationFactory
...
print(f'main.agent_id={main.agent_id}, talk.agent_id={talk.agent_id}')
print(f'Same scope reused: {factory.for_agent(\"main\") is main}')
"
# EXPECTED: main.agent_id=main, talk.agent_id=aria-talk
#           Same scope reused: True

# 3. Isolation test (requires DB):
# Create session as main, verify aria-talk cannot see it
```

## Prompt for Agent
```
Implement per-agent session isolation for the Aria Engine.

FILES TO READ FIRST:
- MASTER_PLAN.md (lines 87-140 — chat_sessions and chat_messages table schemas)
- aria_engine/agent_pool.py (created in S4-01 — EngineAgent with current_session_id)
- aria_skills/session_manager/__init__.py (existing session manager — patterns to port)
- aria_engine/config.py (created in S1-01 — EngineConfig)

STEPS:
1. Read all files above
2. Create aria_engine/session_isolation.py
3. Implement AgentSessionScope — all queries filtered by agent_id
4. Implement create_session() — auto-bind agent_id
5. Implement get_session() — returns None if wrong agent
6. Implement list_sessions() — always filtered by agent_id
7. Implement add_message() — verifies session ownership before write
8. Implement get_messages() — verifies session ownership
9. Implement SessionIsolationFactory — DI for shared resources
10. Run verification commands

CONSTRAINTS:
- CRITICAL: Every query MUST include agent_id filter — no cross-agent leaking
- Factory pattern: shared DB pool, per-agent scopes
- Message writes must verify session ownership first
```
