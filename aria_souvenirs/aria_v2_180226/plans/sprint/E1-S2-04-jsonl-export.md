# S2-04: JSONL Transcript Export
**Epic:** E1 — Engine Core | **Priority:** P1 | **Points:** 2 | **Phase:** 2

## Problem
OpenClaw stored chat transcripts as `.jsonl` files on disk — one line per message. Several downstream tools and the existing `session_manager` skill expect this format. With sessions now in PostgreSQL, we need an export function that produces backward-compatible JSONL from DB sessions, plus a human-readable Markdown export for review.

## Root Cause
The `session_manager` skill (`aria_skills/session_manager/__init__.py`) reads `.jsonl` transcripts from `/root/.openclaw/agents/{agent}/sessions/{session_id}.jsonl`. When we remove OpenClaw, those files vanish. The `src/api/routers/sessions.py` has no export endpoint — it only syncs from OpenClaw's filesystem. We need a PostgreSQL-backed export that writes JSONL to `aria_memories/exports/` and exposes it via API.

## Fix
### `aria_engine/export.py`
```python
"""
...
async def export_session_markdown(
    ...
    ```
    # Chat Session: {title}
    **Agent:** {agent_id} | **Model:** {model} | **Date:** {date}
    ...
    **Assistant** (12:00:01):
    ...
    ---
    **Session Stats:** 10 messages | 1,500 tokens | $0.001
    ```
...
|---|-----------|---------|-------|
| 1 | 5-layer (DB→ORM→API→api_client→Skills→Agents) | ✅ | Reads DB via ORM, writes to filesystem |
| 2 | .env for secrets (zero in code) | ❌ | No secrets involved |
| 3 | models.yaml single source of truth | ❌ | No model routing — just reads model name from session |
| 4 | Docker-first testing | ✅ | Uses config.memories_path which respects ARIA_MEMORIES_PATH env |
| 5 | aria_memories only writable path | ✅ | Writes only to aria_memories/exports/ |
| 6 | No soul modification | ❌ | No soul access |

## Dependencies
- S1-01 must complete first (aria_engine package)
- S1-05 must complete first (ORM models)
- S2-01 should complete first (ChatEngine creates sessions to export)

## Verification
```bash
# 1. Module imports:
python -c "from aria_engine.export import export_session_jsonl, export_session_markdown, export_session, parse_jsonl_line, read_jsonl_file; print('OK')"
# EXPECTED: OK
...
# Normal message
msg = parse_jsonl_line('{\"role\": \"user\", \"content\": \"Hello\"}')
assert msg['role'] == 'user'
# Metadata line (skipped)
meta = parse_jsonl_line('{\"_session_id\": \"abc\", \"_agent_id\": \"main\"}')
assert meta is None
# Empty line
...
# 3. Export format dispatch:
python -c "
from aria_engine.export import export_session
import asyncio
try:
    asyncio.run(export_session('test', None, None, format='invalid'))
except ValueError as e:
    assert 'Unsupported' in str(e)
    print(f'Validation OK: {e}')
"
# EXPECTED: Validation OK: Unsupported export format: invalid...
```

## Prompt for Agent
```
Implement JSONL and Markdown transcript export from PostgreSQL sessions.

FILES TO READ FIRST:
- aria_engine/config.py (EngineConfig.memories_path — created in S1-01)
- aria_engine/exceptions.py (SessionError — created in S1-01)
- src/api/db/models.py (EngineChatSession, EngineChatMessage — created in S1-05)
- aria_skills/session_manager/__init__.py (lines 50-70 — _archive_transcript, understand .jsonl format)

STEPS:
1. Read all files above
2. Create aria_engine/export.py
3. Implement export_session_jsonl() — OpenAI-compatible JSONL with metadata header
4. Implement export_session_markdown() — human-readable with collapsible thinking
5. Implement export_session() — format dispatcher
6. Implement parse_jsonl_line() and read_jsonl_file() — import utilities
7. Ensure files are saved to aria_memories/exports/{agent_id}/
8. Run verification commands

CONSTRAINTS:
- Constraint 5: Only write to aria_memories/exports/
- JSONL format must be backward-compatible with OpenClaw's .jsonl files
- Metadata header line uses '_' prefixed keys (skipped by parsers)
- Handle encoding properly (ensure_ascii=False for Unicode)
```
