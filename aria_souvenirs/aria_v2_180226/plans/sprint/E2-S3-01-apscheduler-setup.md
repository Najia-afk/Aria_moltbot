# S3-01: APScheduler 4.x with PostgreSQL Job Store
**Epic:** E2 — Scheduler & Heartbeat | **Priority:** P0 | **Points:** 5 | **Phase:** 2

## Problem
Aria's 15 cron jobs are defined in `aria_mind/cron_jobs.yaml` but require OpenClaw's Node.js scheduler to execute. Without OpenClaw, no jobs run — work cycles stop, social posts don't fire, reflections never happen. We need a native Python scheduler with persistent job state in PostgreSQL so jobs survive container restarts and can be managed from the web UI.

Reference: `aria_mind/cron_jobs.yaml` defines 15 jobs with cron expressions, agent assignments, and session modes. The `aria_engine.cron_jobs` table (from S1-05 migration) stores job definitions. APScheduler 4.x provides async-native scheduling with pluggable data stores.

## Root Cause
OpenClaw's built-in cron system (`openclaw cron add`) was the only scheduler. It ran inside the Node.js clawdbot container and executed jobs by sending prompts to agents. No Python-native scheduler was ever built because OpenClaw handled it. Now that we're removing OpenClaw, we need `EngineScheduler` to take over — reading job definitions from PostgreSQL and dispatching them to agents via the `AgentPool`.
...
class EngineScheduler:
    ...
    async def _dispatch_to_agent(
        ...
        if self._agent_pool is None:
            raise SchedulerError(f"AgentPool not available — cannot execute job {job_id}")
...
|---|-----------|---------|-------|
| 1 | 5-layer (DB→ORM→API→api_client→Skills→Agents) | ✅ | Scheduler at infrastructure layer, dispatches to agents |
| 2 | .env for secrets (zero in code) | ✅ | DATABASE_URL from env via EngineConfig |
| 3 | models.yaml single source of truth | ❌ | No model references in scheduler |
| 4 | Docker-first testing | ✅ | PostgreSQL required for job store |
| 5 | aria_memories only writable path | ❌ | No file writes — all state in DB |
| 6 | No soul modification | ❌ | No soul access |

## Dependencies
- S1-01 (aria_engine package structure, config, exceptions)
- S1-05 (Alembic migration creating aria_engine.cron_jobs table)
- S4-01 (AgentPool — for dispatching jobs to agents; can stub during dev)

## Verification
```bash
# 1. Module imports:
python -c "from aria_engine.scheduler import EngineScheduler, parse_schedule; print('OK')"
# EXPECTED: OK

# 2. Parse schedule expressions:
python -c "
from aria_engine.scheduler import parse_schedule
print(type(parse_schedule('15m')))        # IntervalTrigger
print(type(parse_schedule('0 0 6 * * *')))  # CronTrigger
print(type(parse_schedule('0 */3 * * *')))  # CronTrigger (5-field)
"
# EXPECTED: IntervalTrigger, CronTrigger, CronTrigger
...
engine = create_async_engine('postgresql+asyncpg://localhost/aria')
s = EngineScheduler(EngineConfig(), engine)
print(s.get_status())
"
# EXPECTED: {'running': False, 'active_executions': 0, ...}
```

## Prompt for Agent
```
Implement the EngineScheduler for Aria Engine — APScheduler 4.x with PostgreSQL persistence.

FILES TO READ FIRST:
- aria_engine/config.py (created in S1-01 — EngineConfig class)
- aria_engine/exceptions.py (created in S1-01 — SchedulerError)
- aria_mind/cron_jobs.yaml (full file — 15 existing cron jobs with schedule formats)
- MASTER_PLAN.md (lines 87-155 — aria_engine schema, cron_jobs table definition)
- aria_engine/__init__.py (created in S1-01 — package structure)

STEPS:
1. Read all files above
2. Create aria_engine/scheduler.py with EngineScheduler class
3. Implement parse_schedule() — convert cron expressions + interval shorthand to APScheduler triggers
4. Implement start()/stop() lifecycle with APScheduler async context
5. Implement _load_jobs_from_db() — read aria_engine.cron_jobs, register with APScheduler
6. Implement _execute_job() — concurrency control, timeout, retry with backoff
7. Implement _dispatch_to_agent() — route to agent via AgentPool
8. Implement CRUD: add_job(), update_job(), remove_job(), trigger_job()
9. Implement get_job(), list_jobs(), get_job_history()
10. Run verification commands

CONSTRAINTS:
- Constraint 1: Scheduler is infrastructure layer — dispatches to agents, doesn't call skills directly
- Constraint 2: DATABASE_URL from environment via EngineConfig
- Constraint 4: Requires PostgreSQL (Docker) for integration testing
```
